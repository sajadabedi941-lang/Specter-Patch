...
SKIP_DIR_NAMES = {
    "Release", "__pycache__", ".git", "backup", "Backup", "cache", "Cache",
}
SKIP_SUFFIXES = {".pyc", ".pyo", ".bak", ".tmp", ".old", ".orig", ".swp"}
...
def package_files() -> list[Path]:
    ...
    for path in ROOT.rglob("*"):
        ...
        if path.suffix.lower() in SKIP_SUFFIXES:
            continue
        # stale hash sidecars / old manifests inside tree
        name = path.name.lower()
        if name.endswith(".sha256") and path.name != "SYNC_MANIFEST.sha256":
            continue
        if name.endswith(".bak") or name.endswith(".old"):
            continue
        files.append(path)
