#!/usr/bin/env python3
"""BUILDING_FLAG_STAGE_03_VIETNAM_BARRACKS_ONLY.

Baseline: BUILDING_FLAG_STAGE_02_JAPAN_BARRACKS_ONLY (approved Japan pole).

Vietnam_Barracks is the next irq_camp / baked-Iraqi-FLAG target.
Same safe method as Japan_Barracks:
  - ModuleTag_01 Model=/Animation= irq_camp untouched
  - One Abbas-style ModuleTag_03 Flag_Hs Draw
  - One new W3D VN__VNFlag_Hs.W3D from Irq__IqFlag_Hs
    packed name == internal VN__VNFLAG_HS
    texture VN_Flag.tga (already packed)

Does not touch irq_camp, IraqiFlag, DPRK_Flag, donor Flag_Hs,
Japan_Barracks, JP__JPFlag_Hs, PlayerTemplate, HUD, or pages.
"""
...
SRC_DATA = Path("/workspace/patch/Release/BUILDING_FLAG_STAGE_02_JAPAN_BARRACKS_ONLY/_SPEC_DATA_ONE.big")
SRC_ART = Path("/workspace/patch/Release/BUILDING_FLAG_STAGE_02_JAPAN_BARRACKS_ONLY/_SPEC_ART_ONE.big")
SRC_DATA_SHA = "4828413b555ab327f20d63e7236d6674304c8abcd4849c3a72fcb11b351aadee"
SRC_ART_SHA = "75048828b4d2dac692c5d7d5fa251c42c8b536e9ed0363a0747bcba0d2e1a59f"

OUT_DIR = Path("/tmp/BUILDING_FLAG_STAGE_03_VIETNAM_BARRACKS_ONLY")
WS_OUT = Path("/workspace/patch/Release/BUILDING_FLAG_STAGE_03_VIETNAM_BARRACKS_ONLY")
RELEASE_NAME = "BUILDING_FLAG_STAGE_03_VIETNAM_BARRACKS_ONLY"
...
P_NEW_W3D = r"Art\W3D\VN__VNFlag_Hs.W3D"
...
NEW_NAME = b"VN__VNFLAG_HS"
DONOR_TEX = b"IraqiFlag.dds"
NEW_TEX = b"VN_Flag.tga"
...
def make_vn_flag_hs(donor: bytes) -> bytes:
    ...
    if b"IRQ__IQFLAG_HS" in out or b"NKR__NKFLAG_HS" in out or b"JP__JPFLAG_HS" in out:
        raise SystemExit("other Flag_Hs container leaked into new W3D")
    if b"IraqiFlag" in out or b"DPRK_Flag" in out or b"JP_Flag" in out:
...
FLAG_DRAW = """  ; ------------ Flag -----------------
  Draw                = W3DModelDraw ModuleTag_03
    ConditionState    = None
      Model           = VN__VNFlag_Hs
      Animation       = VN__VNFlag_Hs.VN__VNFlag_Hs
      AnimationMode   = LOOP
    End
...
def patch_vietnam_barracks(text: str) -> str:
    if "ModuleTag_03" in text:
        raise SystemExit("Vietnam_Barracks already has ModuleTag_03")
    if "HideSubObject" in text:
        raise SystemExit("HideSubObject must not be added")
    if "Object Vietnam_Barracks" not in text:
        raise SystemExit("missing Object Vietnam_Barracks")
    nl = jf.file_nl(text)
    draw = jf.to_nl(FLAG_DRAW, nl)
    m = re.search(r"(?im)^[ \t]*PlacementViewAngle\s*=", text)
    if not m:
        raise SystemExit("Vietnam_Barracks missing PlacementViewAngle insert point")
    prefix = text[: m.start()]
    ...
    if new.count("VN__VNFlag_Hs") != 9:
        raise SystemExit(f"VN__VNFlag_Hs refs {new.count('VN__VNFlag_Hs')}")
    if len(re.findall(r"(?im)^\s*Model\s*=\s*VN__VNFlag_Hs\s*$", new)) != 3:
        raise SystemExit("expected 3 ModuleTag_03 Model=VN__VNFlag_Hs")
    if len(re.findall(r"(?im)^\s*Animation\s*=\s*VN__VNFlag_Hs\.VN__VNFlag_Hs\s*$", new)) != 3:
        raise SystemExit("expected 3 ModuleTag_03 Animation=VN__VNFlag_Hs.VN__VNFlag_Hs")
...
def main() -> int:
    ...
    vn_tga = art_of(art, P_VN_TGA)
    if len(vn_tga) < 18 or vn_tga[2] != 2:
        raise SystemExit("VN_Flag.tga is not uncompressed truecolor")

    new_w3d = make_vn_flag_hs(donor)
    add_bytes(art, P_NEW_W3D, new_w3d)

    barracks = jf.text_of(data, P_VN_BARRACKS)
    jf.set_text(data, P_VN_BARRACKS, patch_vietnam_barracks(barracks))
    ...
    for p in (
        P_DONOR_W3D, P_NK_W3D, P_JP_W3D, P_IRQ_CAMP, P_IRQ_DDS,
        P_DPRK_TGA, P_DPRK_DDS, P_VN_TGA, P_JP_TGA,
    ):
        key = jf.norm(p).lower()
    ...
    changed_data = [n for n, b in new_data_map.items() if src_data_map.get(n) != b]
    if changed_data != [jf.norm(P_VN_BARRACKS).lower()]:
        raise SystemExit(f"unexpected DATA changes: {changed_data}")
    for frozen in (P_PT, P_PT_PATCH, P_REMAIN, P_CAMP, P_JP_BARRACKS, P_CMDBTN, P_CMDSET):
    ...
    w3d_names = [
        n for n, _ in art if jf.norm(n).lower().endswith("vn__vnflag_hs.w3d")
    ]
    if len(w3d_names) != 1:
        raise SystemExit(f"duplicate VN Flag_Hs path: {w3d_names}")
    ...
    changed = "\n".join(
        [
            "BUILDING_FLAG_STAGE_03_VIETNAM_BARRACKS_ONLY changed files",
            "",
            "TARGET AUDIT",
            "  Object = Vietnam_Barracks (unique)",
            f"  INI = {P_VN_BARRACKS}",
            "  Model= irq_camp  Animation= irq_camp.irq_camp",
            "  Baked flags = IRQ_CAMP.FLAG01/02/03 + IraqiFlag.tga",
            "  Shared irq_camp users include Iraq/Egypt/Israel (protected) — mesh not edited",
            "  Flag_Hs safe = YES (second Draw only, Japan method)",
            "",
            "DATA (1 file)",
            f"  {P_VN_BARRACKS}",
            "    Object Vietnam_Barracks",
            "    ModuleTag_01 UNCHANGED Model=irq_camp Animation=irq_camp.irq_camp",
            "    Added ModuleTag_03 Draw from Japan_Barracks / Iraq_Abbas pattern",
            "    Model=VN__VNFlag_Hs Animation=VN__VNFlag_Hs.VN__VNFlag_Hs",
            "    HideSubObject = NO",
            "",
            "ART (1 new file)",
            f"  {P_NEW_W3D}",
            "    donor=Art\\W3D\\Irq__IqFlag_Hs.W3D (bytes copied, donor file unmodified)",
            "    internal=VN__VNFLAG_HS  packed=VN__VNFlag_Hs  MATCH",
            "    texture=VN_Flag.tga (already packed)",
            "",
            "UNCHANGED",
            "  Japan_Barracks JP__JPFlag_Hs.W3D",
            "  irq_camp.W3D IraqiFlag.dds Irq__IqFlag_Hs.W3D NKr__NKFlag_Hs.W3D",
    ...
    audit = "\n".join(
        [
            "BUILDING_FLAG_STAGE_03_VIETNAM_BARRACKS_ONLY",
            "BASELINE = BUILDING_FLAG_STAGE_02_JAPAN_BARRACKS_ONLY",
            ...
            "INTERNAL_NAME = VN__VNFLAG_HS",
            "INTERNAL_LEN = 12",
            "TEXTURE = VN_Flag.tga",
            "FIX_APPLIED = YES (Vietnam_Barracks Flag_Hs pole only)",
            "BOOT_SAFE = YES",
            "INGAME_TESTED = NO",
            "NEXT = wait for real ZH launch of Vietnam_Barracks only",
        ]
    )
    changelog = """BUILDING_FLAG_STAGE_03_VIETNAM_BARRACKS_ONLY

Next isolated country/building after the approved Japan_Barracks pole.

Vietnam_Barracks uses the same irq_camp mesh with baked Iraqi
FLAG01/FLAG02/FLAG03. This pair adds one Vietnam Flag_Hs pole only.
ModuleTag_01 Model=/Animation= are unchanged. Shared irq_camp is
not edited. Japan_Barracks and JP__JPFlag_Hs stay as Stage 02.

One new W3D: VN__VNFlag_Hs.W3D, authored from Irq__IqFlag_Hs with
internal container VN__VNFLAG_HS (matches packed name) and texture
VN_Flag.tga. Donor Iraq/NK Flag_Hs files are not modified.

Wait for a real ZH launch of Vietnam Barracks before the next country.
"""
