#!/usr/bin/env python3
"""DATA-only Iraq unlock + air pass on the PR #473 packed DATA BIG.

Starts from /tmp/pr473_baseline/_SPEC_DATA_ONE.big (PR #473).
...
SRC_DATA = Path("/tmp/pr473_baseline/_SPEC_DATA_ONE.big")
OUT_DIR = Path("/tmp/iraq_unlock")
REPO_OVERLAY = Path("/workspace/patch/Data/INI/Object/Specter/IraqUnlock")
REPO_DOCS = Path("/workspace/patch/Release/docs")
...
def set_initial_commandset(text: str, new_cs: str) -> str:
    return re.sub(
        r"(?im)^(?P<ind>[ \t]*)CommandSet\s*=\s*\S+",
        lambda m: f"{m.group('ind')}CommandSet       = {new_cs}"
        if m.start() == text.lower().find("commandset")
        else m.group(0),
        text,
        count=1,
    )
...
def insert_cs_slot(cs_text: str, cs_name: str, slot: str, button: str) -> str:
    nl = "\r\n" if "\r\n" in cs_text else "\n"
    pat = re.compile(rf"(?im)^(CommandSet {re.escape(cs_name)}\s*\r?\n)(.*?)(^End\s*\r?\n)", re.S)
    ...
    if re.search(rf"(?im)^\s*{re.escape(slot)}\s*=", body):
        ...
        # insert after previous numeric slot if present, else after header
        prev = str(int(slot) - 1)
...
def make_iraq_tu22(rus_text: str) -> str:
    ...
    # insert empty Prerequisites after Side
...
def main() -> int:
    if not SRC_DATA.is_file():
        raise SystemExit(f"missing PR #473 DATA BIG {SRC_DATA}")
    ...
    for name, blob in src:
        ...
        if key == CS_INI:
            ...
            # Iraq-only CommandSet mutation guard
    ...
    (REPO_OVERLAY / "IraqJetL159_WpnBomb.ini").write_text(L159_WEAPON.replace("\n", "\r\n"), encoding="latin1")
    (OUT_DIR / "SHA256.txt").write_text(
        "DATA  _SPEC_DATA_ONE.big\n"
        f"SHA256 {dsha}\n"
        "ART    unchanged inherited Specter_Aircraft_Global_Update_Final\n"
