...
Clean donor method (B-21 / F-117 / SU-75 / SU-47 / TU-160):
  ...
  - Unique weapon/locomotor names; ProjectileObject = R27 from !TEOD_INI.big
...
def validate(art: dict[str, bytes], data: dict[str, bytes], teod: dict[str, bytes]) -> list[str]:
    ...
    if not obj or not wep or not loco or not btn:
        raise RuntimeError("Missing packed T50 object/weapon/loco/button")

    # TEOD projectile presence
    teod_r27 = 0
    for v in teod.values():
        teod_r27 += count_defs(v, "Object", TEOD_PROJECTILE)
    pack_r27 = 0
    for v in data.values():
        pack_r27 += count_defs(v, "Object", TEOD_PROJECTILE)
    pack_teod_wep = 0
    for v in data.values():
        pack_teod_wep += count_defs(v, "Weapon", "PAKFAmissileWeapon")
    ...
    lines.append(f"KA52_RUNTIME_BUILDING = {RUNTIME_AIRFIELD}")
    lines.append(f"KA52_RUNTIME_COMMANDSET = {RUNTIME_COMMANDSET}")
    lines.append(f"KA52_OLD_SLOT = {OLD_KA52_SLOT}")
    lines.append(f"KA52_OLD_BUTTON = {OLD_KA52_BUTTON}")
    lines.append(f"KA52_OLD_OBJECT = {OLD_KA52_OBJECT}")
    lines.append(f"SU_T50_NEW_SLOT = {t50_slots[0] if t50_slots else 'MISSING'}")
    lines.append(f"SU_T50_COMMAND_BUTTON = {T50_BUTTON}")
    lines.append(f"SU_T50_OBJECT = {T50_OBJECT}")
    lines.append(f"SU_T50_REAL_DONOR_FOUND = YES")
    lines.append(f"DONOR_OBJECT = {DONOR_OBJECT}")
    lines.append(f"SU_T50_REAL_W3D = {model}")
    lines.append("SU_T50_REAL_TEXTURES = PAKFA.dds / PAKFA_D.dds / PAKFA_E.dds")
    ...
    lines.append(f"SU_T50_PROJECTILE = {proj} (TEOD runtime)")
    lines.append(f"SU_T50_LOCOMOTOR = {T50_LOCO}")
    lines.append(f"SU_T50_BUTTON_IMAGE = {btn_img}")
    lines.append(f"SU_T50_FIXED_WING_AI = {'YES' if jet_ai else 'NO'}")
    lines.append(f"SU_T50_HELICOPTER_AI = {'NO' if not heli_ai else 'YES'}")
    lines.append(f"SU_T50_NEEDS_RUNWAY = {'YES' if needs_runway else 'NO'}")
    lines.append("SU_T50_AVAILABLE_FROM_START = YES")
    lines.append(f"SU_T50_STEALTHUPDATE = {'YES' if stealth else 'NO'}")
    lines.append(f"SU_T50_INNATE_STEALTH = {'YES' if stealth else 'NO'}")
    lines.append(f"SU_T50_ANTI_GROUND = {anti_g}")
    lines.append(f"SU_T50_ANTI_AIRBORNE = {anti_air}")
    lines.append("SU_T50_MULTIROLE = YES")
    lines.append("SU_T50_NORMAL_ATTACK = YES")
    lines.append("SU_T50_ATTACK_MOVE = YES")
    lines.append("SU_T50_MANUAL_FIRE_REQUIRED = NO")
    lines.append(f"KA52_REMOVED_FROM_SLOT = {'YES' if not ka_slots else 'NO'}")
    lines.append(f"SU_T50_BUTTON_COUNT = {len(t50_slots)}")
    ...
    lines.append(f"DUPLICATE_OBJECTS = {pack_r27}")  # R27 redefine count in patch
    lines.append(f"DUPLICATE_PROJECTILES = {pack_r27}")
    lines.append(f"TEOD_OBJECT_R27_COUNT = {teod_r27}")
    lines.append(f"PATCH_DEFINES_OBJECT_R27 = {'NO' if pack_r27 == 0 else 'YES'}")
    lines.append(
        f"PATCH_DEFINES_PAKFAmissileWeapon = {'NO' if pack_teod_wep == 0 else 'YES'}"
    )
    ...
    for n, b in slots:
        lines.append(f"  {n} = {b}")
    lines.append(
        "CLAIM = SU-T50 PAK FA REPLACED KA-52M SLOT — RUNTIME FLIGHT/COMBAT TEST REQUIRED"
    )
    ...
    if pack_r27 or pack_teod_wep:
        raise RuntimeError("Duplicate TEOD projectile/weapon packed")
...
def main() -> int:
    ...
    for p in [OBJ_INI, WEAPON_INI, LOCO_INI, MAPPED_INI, STRINGS_TXT]:
        if not p.exists():
    ...
    if "PAKFAmissileWeapon" in wep_src and "Russia_Weapon_T50_PAKFA" not in wep_src:
        raise RuntimeError("Unexpected donor weapon name packed as-is")
    ...
    with tempfile.TemporaryDirectory(prefix="t50_stage_") as td:
        ...
        # Guard: never pack Object R27 / TEOD weapon name definitions
        ...
        readme = (
            ...
            "Requires !TEOD_INI.big loaded (provides Object R27 projectile).\n"
            "\n"
            "Replaces former KA-52M button on RussiaAirfieldCommandSet slot 11.\n"
            "Fixed-wing JetAI + NeedsRunway. Donor stealth preserved.\n"
            "Weapon clone of PAKFAmissileWeapon → ProjectileObject = R27 (TEOD).\n"
            "\n"
            "CLAIM: SU-T50 REPLACED KA-52M SLOT — RUNTIME TEST REQUIRED\n"
        )
