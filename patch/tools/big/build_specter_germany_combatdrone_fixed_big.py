#!/usr/bin/env python3
"""Patch Germany_CombatDrone.ini inside _SPEC_DATA_ONE.big (USA MQ9 donor).

Same crash pattern as Britain/France CombatDrone:
...
Source: SPECTER_SPEC_DATA_ONE_FRANCE_DRONE_FIXED/_SPEC_DATA_ONE.big
"""
...
SRC = ROOT / "Release" / "SPECTER_SPEC_DATA_ONE_FRANCE_DRONE_FIXED" / "_SPEC_DATA_ONE.big"
ART = ROOT / "Release" / "SPECTER_BIG_MERGE" / "_SPEC_ART_ONE.big"
OUT = ROOT / "Release" / "SPECTER_SPEC_DATA_ONE_GERMANY_DRONE_FIXED"
GERMANY_PATH = (
...
def clone_usa_to_germany(usa_text: str) -> str:
    ...
    text = text.replace("Object AmericaDronesMq9", "Object Germany_CombatDrone", 1)
    text = re.sub(r"(?m)^(  Side\s*=\s*)America\s*$", r"\1Germany", text, count=1)
    text = re.sub(
        r"(?m)^(  DisplayName\s*=\s*)OBJECT:\S+\s*$",
        r"\1OBJECT:Germany_CombatDrone",
        text,
        count=1,
    )
    text = re.sub(
        r"(?ms)^  Prerequisites\n.*?^  End",
        "  Prerequisites\n"
        "    Object = Germany_AdvancedAirBase\n"
    ...
    text = re.sub(
        ...
        r"\1Germany_Weapon_ATGM",
        text,
        count=1,
    )
    text = re.sub(r"(?m)^(  BuildCost\s*=\s*)\S+", r"\g<1>1068", text, count=1)
    text = re.sub(r"(?m)^(  BuildTime\s*=\s*)\S+", r"\g<1>11.0", text, count=1)
    ...
    header = (
        "; SPECTER FIX - Germany_CombatDrone\n"
        "; Donor: USA AmericaDronesMq9 (Mq9.ini) - same pattern as Britain/France CombatDrone fix\n"
        "; Keep: Object Germany_CombatDrone / Side=Germany / Germany_AdvancedAirBase\n"
        "; Weapons: Germany_Weapon_ATGM + 2x_GBU12II_Mq9\n"
...
def main() -> int:
    ...
    if knorm(GERMANY_PATH) not in by:
        raise SystemExit("Germany_CombatDrone.ini missing from source BIG")
    if knorm(USA_PATH) not in by:
        raise SystemExit("USA Mq9.ini missing from source BIG")

    broken = by[knorm(GERMANY_PATH)][1]
    usa_text = by[knorm(USA_PATH)][1].decode("utf-8", "replace")
    fixed = clone_usa_to_germany(usa_text)
    ...
    chk("Object=Germany_CombatDrone", bool(re.search(r"(?m)^Object\s+Germany_CombatDrone\b", fixed)))
    chk("Side=Germany", bool(re.search(r"(?m)^  Side\s*=\s*Germany\s*$", fixed)))
    ...
    chk("BuildCost=1068", bool(re.search(r"(?m)^  BuildCost\s*=\s*1068\s*$", fixed)))
    chk("BuildTime=11.0", bool(re.search(r"(?m)^  BuildTime\s*=\s*11\.0\s*$", fixed)))
    ...
    for name, blob in entries:
        if knorm(name) == knorm(GERMANY_PATH):
    ...
    rebuilt = rebuilt_by[knorm(GERMANY_PATH)][1]
    if rebuilt != fixed_bytes:
        raise SystemExit("round-trip Germany content mismatch")
    ...
    for n, b in rebuilt_entries:
        k = knorm(n)
        if k == knorm(GERMANY_PATH):
            continue
        if src_by[k] != b:
            raise SystemExit(f"other file changed: {n}")

    # Preserve prior France fix inside this BIG
    france_path = (
        r"Data\INI\Object\Specter\French Armed Forces\Drones\France_CombatDrone.ini"
    )
    france_sha = sha256_bytes(rebuilt_by[knorm(france_path)][1])
    if france_sha != "7512cca46c234c6951a54d5a982184209d203668738c4e1336fdaab1e1ba8df2":
        raise SystemExit(f"France fix lost: {france_sha}")

    TREE.parent.mkdir(parents=True, exist_ok=True)
    TREE.write_bytes(fixed_bytes)
    (OUT / "Germany_CombatDrone.ini").write_bytes(fixed_bytes)

    big_sha = sha256_file(out_big)
    big_size = out_big.stat().st_size
    germany_sha = sha256_bytes(fixed_bytes)
    broken_sha = sha256_bytes(broken)

    report = (
        "SPECTER GERMANY COMBATDRONE FIXED - VERIFY REPORT\n"
        "============================================================\n"
        f"VERDICT: {'PASS' if ok_all else 'FAIL'}\n"
        "Scope: Germany_CombatDrone.ini patched INSIDE _SPEC_DATA_ONE.big\n"
        "Overlay-only: NO (file is inside DATA BIG)\n"
        f"\nSource BIG (France drone fixed): {sha256_file(SRC)}\n"
        f"Patched BIG SHA256: {big_sha}\n"
        f"Patched BIG SIZE:   {big_size}\n"
        f"Broken Germany SHA: {broken_sha}\n"
        f"Fixed Germany SHA:  {germany_sha}\n"
        f"Fixed Germany SIZE: {len(fixed_bytes)}\n"
        f"France fix preserved: {france_sha}\n"
        "\nRoot causes (same as Britain/France CombatDrone):\n"
        "  - Non-ASCII UTF-8 comments (parser crash)\n"
        "  - Science prereq SCIENCE_UAEStealthJet (not a defined Science)\n"
        "  - Duplicate Shadow = SHADOW_VOLUME\n"
        "\nRepair (USA MQ9 donor):\n"
        "  Object=Germany_CombatDrone Side=Germany\n"
        "  CommandSet=GenericTacticalBomberCommandSet\n"
        "  Weapons=Germany_Weapon_ATGM + 2x_GBU12II_Mq9\n"
        "  Models=US_MQ9 / US_MQ9D / US_MQ9R\n"
        "  Prereq=Germany_AdvancedAirBase + SCIENCE_Rank3\n"
        "  BuildCost=1068 BuildTime=11.0\n"
    ...
    (OUT / "README_INSTALL.txt").write_text(
        "SPECTER GERMANY COMBATDRONE FIXED\n"
        "=================================\n\n"
        "This package replaces Data\\_SPEC_DATA_ONE.big with Germany_CombatDrone\n"
        "already patched INSIDE the archive (not an external overlay).\n"
        "Also retains the prior France_CombatDrone in-BIG fix.\n\n"
        "INSTALL:\n"
        ...
        "5. Launch and confirm Germany Combat Drone no longer crashes init.\n",
        encoding="ascii",
    )
    hashes = (
        f"_SPEC_DATA_ONE.big SHA256={big_sha} SIZE={big_size}\n"
        f"Germany_CombatDrone.ini SHA256={germany_sha} SIZE={len(fixed_bytes)}\n"
        f"Broken vendor Germany_CombatDrone.ini SHA256={broken_sha}\n"
        f"Source France-fixed BIG SHA256={sha256_file(SRC)}\n"
        f"France_CombatDrone.ini SHA256={france_sha} (preserved)\n"
    )
    ...
    zpath = OUT / "_SPEC_DATA_ONE_GERMANY_DRONE_FIXED.zip"
    with zipfile.ZipFile(zpath, "w", zipfile.ZIP_DEFLATED) as zf:
        zf.write(out_big, "_SPEC_DATA_ONE.big")
        zf.write(OUT / "Germany_CombatDrone.ini", "Germany_CombatDrone.ini")
