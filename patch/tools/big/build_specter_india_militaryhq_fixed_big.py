#!/usr/bin/env python3
"""Repair India_MilitaryHQ inside _SPEC_DATA_ONE.big (USA CommandCenter / Egypt MHQ method).

Same validation pattern as India_CommandCenter fix:
- Patch INSIDE the DATA BIG (not loose-only)
- Full Object/Draw/Shadow/ModuleTag/End/W3D/ref validation
- Extract-from-BIG verify after write; release only on PASS

Donor: AmericaCommandCenter (same working MilitaryHQ structure as verified
Egypt_MilitaryHQ). Keep India Object/Side/CommandSet/naming.
"""
...
SRC = ROOT / "Release" / "SPECTER_SPEC_DATA_ONE_INDIA_CC_FIXED" / "_SPEC_DATA_ONE.big"
ART = ROOT / "Release" / "SPECTER_BIG_MERGE" / "_SPEC_ART_ONE.big"
OUT = ROOT / "Release" / "SPECTER_SPEC_DATA_ONE_INDIA_MHQ_FIXED"
...
TREE = (
    ROOT
    / "Data/INI/Object/Specter/Indian Armed Forces/Buildings/India_MilitaryHQ.ini"
)
...
def clone_usa_to_india_mhq(usa_text: str) -> str:
    ...
    header = (
        "; SPECTER FIX - India_MilitaryHQ\n"
        "; Donor: AmericaCommandCenter (same method as verified Egypt_MilitaryHQ)\n"
        "; Keep: Object India_MilitaryHQ / Side=India / India_MilitaryHQCommandSet\n"
        "; Art/behaviors from USA CC (US_Command); no Irq/Adnan crash modules\n"
    ...
    if "Object India_MilitaryHQ" not in text:
        raise SystemExit("Object India_MilitaryHQ missing")
    if not re.search(r"(?m)^  Side\s*=\s*India\s*$", text):
        raise SystemExit("Side India missing")
    if "India_MilitaryHQCommandSet" not in text:
        raise SystemExit("India MilitaryHQ CommandSet missing")
    if CRASH_TOKENS.search(text):
        raise SystemExit(f"Irq crash tokens remain: {CRASH_TOKENS.findall(text)}")
...
def main() -> int:
    ...
    if knorm(MHQ_PATH) not in by:
        raise SystemExit("India_MilitaryHQ missing from source BIG")

    usa_raw = by[knorm(USA_PATH)][1]
    usa_text = usa_raw.decode("utf-8", "replace")
    old_raw = by[knorm(MHQ_PATH)][1]
    ...
    shadows = re.findall(r"(?m)^\s*Shadow\s*=\s*(\S+)", old_text)
    if len(shadows) != 1:
        old_fails.append(f"Shadow={shadows}")
    print("OLD India_MilitaryHQ issues:", old_fails or ["none flagged"])
    ...
    repl = clone_usa_to_india_mhq(usa_text)
    tmp_entries = [(n, b) for n, b in entries if knorm(n) != knorm(MHQ_PATH)]
    tmp_entries.append((MHQ_PATH, repl.encode("ascii")))
    repl_fails = validate_building_ini(
        repl,
        expect_object="India_MilitaryHQ",
        expect_side="India",
        entries=tmp_entries,
        art_entries=art_entries,
        label="INDIA_MHQ",
    ...
    if repl_fails:
        ...
        for f in repl_fails:
            print(" ", f)
        return 1
    print("PASS India_MilitaryHQ full validation (pre-write)")

    new_entries: list[tuple[str, bytes]] = []
    replaced = False
    for name, blob in entries:
        if knorm(name) == knorm(MHQ_PATH):
            new_entries.append((name, repl.encode("ascii")))
            replaced = True
        else:
            new_entries.append((name, blob))
    if not replaced:
        raise SystemExit("failed to replace India_MilitaryHQ entry")
    ...
    chk("replaced India_MilitaryHQ", replaced)
    chk("Object India_MilitaryHQ", "Object India_MilitaryHQ" in repl)
    chk("Side India", bool(re.search(r"(?m)^  Side\s*=\s*India\s*$", repl)))
    chk("India CommandSet", "India_MilitaryHQCommandSet" in repl)
    ...
    chk("Draw present", bool(re.search(r"(?m)^\s*Draw\s*=\s*W3DModelDraw\b", repl)))
    ...
    if knorm(MHQ_PATH) not in rby:
        raise SystemExit("EXTRACT FAIL: India_MilitaryHQ missing")
    ename, ebytes = rby[knorm(MHQ_PATH)]
    ...
    extract_path = extract_dir / "India_MilitaryHQ.ini"
    ...
    if not (len(changed) == 1 and knorm(changed[0]) == knorm(MHQ_PATH)):
    ...
    (OUT / "India_MilitaryHQ.ini").write_bytes(repl_bytes)
    ...
    report = (
        "SPECTER INDIA MILITARY HQ (USA DONOR / EGYPT MHQ METHOD) - VERIFY REPORT\n"
        "=======================================================================\n"
        "VERDICT: PASS\n"
        "Patched INSIDE: _SPEC_DATA_ONE.big\n"
        "Path: Data\\INI\\Object\\Specter\\Indian Armed Forces\\Buildings\\India_MilitaryHQ.ini\n"
        "Donor: AmericaCommandCenter (same method as verified Egypt_MilitaryHQ)\n"
        "Keep: Object/Side=India/India_MilitaryHQCommandSet/DisplayName\n"
        "Removed: Irq_Command, irq_comndcntr, Iraq_Adnan1, SUPERWEAPON_Iraqi*, non-ASCII\n"
        f"\nOld India_MHQ SHA256: {sha256_bytes(old_raw)}\n"
        f"New India_MHQ SHA256: {repl_sha}\n"
        ...
        "Preserved: France/Germany CombatDrone, India_EliteStrike, India_CommandCenter\n"
    ...
    (OUT / "EMBED_PROOF.txt").write_text(
        "EMBED + EXTRACT TEST\n"
        "====================\n"
        f"India_MilitaryHQ entry: {ename}\n"
        ...
        f"Irq_Adnan_tokens: NONE\n"
        f"BIG_sha256: {big_sha}\n"
        f"BIG_size: {big_size}\n",
        encoding="ascii",
    )
    (OUT / "README_INSTALL.txt").write_text(
        "SPECTER INDIA MILITARY HQ FIX\n"
        "=============================\n\n"
        "India_MilitaryHQ patched INSIDE _SPEC_DATA_ONE.big.\n"
        "Rebuilt from USA AmericaCommandCenter (Egypt_MilitaryHQ method).\n"
        "Side=India, India_MilitaryHQCommandSet, US_Command art.\n"
        "All Irq/Adnan crash tokens removed.\n\n"
        "Install: replace Data\\_SPEC_DATA_ONE.big; keep _SPEC_ART_ONE.big.\n",
        encoding="ascii",
    )
    (OUT / "HASHES.txt").write_text(
        f"_SPEC_DATA_ONE.big SHA256={big_sha} SIZE={big_size}\n"
        f"India_MilitaryHQ.ini SHA256={repl_sha}\n"
    ...
    with zipfile.ZipFile(final_zip, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        zf.write(OUT / "India_MilitaryHQ.ini", "India_MilitaryHQ.ini")
        zf.write(extract_path, "EXTRACT_VERIFY/India_MilitaryHQ.ini")
    ...
    zpath = OUT / "_SPEC_DATA_ONE_INDIA_MHQ_FIXED.zip"
    if zpath.exists():
        zpath.unlink()
