#!/usr/bin/env python3
"""Pack patch/Data + patch/Art into standalone _SPECTER_PATCH_FINAL.big overlay.
...
def main() -> int:
    ...
    ap.add_argument("--out", type=Path, required=True, help="Path to _SPECTER_PATCH_FINAL.big")
    args = ap.parse_args()
    ...
    print(f"Wrote {args.out} entries={len(file_map)} bytes={len(data)} SHA256={hashlib.sha256(data).hexdigest()}")
    return 0
